...
/**
 * Represents a byte stream for DICOM files. Extends the default FileReader
 * class. Contains useful functions for handling DICOM files, such as skipHeader
 * or getByte() or the current byte position in the file.
 * 
...
public class DiFileInputStream extends FileInputStream {
	...
	/**
	 * Reads bytes.length bytes and is much faster than calling read() bytes.length
	 * times.
	 * 
	...
	/**
	 * Reads a byte from the stream. Returns -1 if end of file is reached & throws
	 * an IOException.
	 * 
	...
	/**
	 * Reads a short (= 2 bytes) from the stream. Returns -1 if end of file is
	 * reached & throws an IOException.
	 * 
	...
	/**
	 * Reads an int (= 4 bytes) from the stream. Returns -1 if end of file is
	 * reached & throws an IOException.
	 * 
	 * @return the int
	 * @throws IOException
	...
	public int getInt() throws IOException {
		...
		if (b0 == -1 || b1 == -1 || b2 == -1 || b3 == -1) {
			...
			result = _little_endian ? ((b3 << 24) + (b2 << 16) + (b1 << 8) + b0)
					: ((b0 << 24) + (b1 << 16) + (b2 << 8) + b3);
		}
	...
	/**
	 * Skips the first 132 bytes. Returns true if it seems to be a dicom file. This
	 * method will be implemented in exercise 1.
	 * 
	...
	public boolean skipHeader() throws IOException {
		...
		for (int i = 0; i < 128; i++)
			read();
		if ((getByte() == 'D') && (getByte() == 'I') && (getByte() == 'C') && (getByte() == 'M'))
			return true;
		else
			return false;
	}
	...
	public static void main(String[] args) throws IOException {
		...
		if (df.skipHeader())
			...
			System.out.println("This is not a DICOM data");
		df.close();
	}
}
